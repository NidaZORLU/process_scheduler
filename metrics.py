...
def calculate_metrics(processes, gantt=None):
    """
    Ortalama waiting/turnaround/response + starvation bilgisi.
    """
    n = len(processes)
    if n == 0:
        return {
    ...
    starved = [p.pid for p in processes if getattr(p, "starvation_risk", False)]

    # CPU Utilization (gantt varsa daha doğru)
    cpu_util = 0.0
    if gantt:
